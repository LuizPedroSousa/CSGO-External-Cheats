...
{
  //Credits to hazedumper for all offsets https://github.com/frk1/hazedumper
  ...
  public async static Task<Root> FetchOffsets()
  {

    Thread.CurrentThread.Priority = ThreadPriority.Highest;
    var data = await new HttpClient().GetStringAsync("https://raw.githubusercontent.com/frk1/hazedumper/master/csgo.min.json");

    if (data is null)
    {
      Console.WriteLine("Failed to load offsets.json");
      Process.GetCurrentProcess().Kill();
    }

    Root offsets = JsonSerializer.Deserialize<Root>(data);


    Console.WriteLine("Fetched offsets of hazedumper");
